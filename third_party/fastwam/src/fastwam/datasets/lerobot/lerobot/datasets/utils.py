...
# v3.0 paths — multi-episode files, paths derived from per-episode metadata
DEFAULT_DATA_PATH_V30 = "data/chunk-{chunk_index:03d}/file-{file_index:03d}.parquet"
DEFAULT_VIDEO_PATH_V30 = "videos/{video_key}/chunk-{chunk_index:03d}/file-{file_index:03d}.mp4"
EPISODES_DIR_V30 = "meta/episodes"
TASKS_PATH_V30 = "meta/tasks.parquet"  # flat single-file in v3.0
_TASKS_V30_CACHE = {}
...
def load_tasks_v30(local_dir: Path) -> tuple[dict, dict]:
    """Load tasks from v3.0 parquet format (meta/tasks.parquet).

    The parquet has index=task_string, column="task_index" (integer).
    Returns (tasks, task_to_task_index) matching the v2.1 interface.
    """
    import pandas as pd
    path = local_dir / TASKS_PATH_V30
    if not path.exists():
        raise FileNotFoundError(f"Tasks parquet not found: {path}")
    stat = path.stat()
    cache_key = (path.name, stat.st_size, stat.st_mtime_ns)
    if cache_key in _TASKS_V30_CACHE:
        return _TASKS_V30_CACHE[cache_key]
    df = pd.read_parquet(path)
    # index = task string, column "task_index" = integer task index
    tasks = {int(row["task_index"]): task_str for task_str, row in df.iterrows()}
    task_to_task_index = {task_str: int(row["task_index"]) for task_str, row in df.iterrows()}
    result = (tasks, task_to_task_index)
    _TASKS_V30_CACHE[cache_key] = result
    return result


def load_episodes_v30(local_dir: Path) -> dict:
    """Load episodes from v3.0 parquet format (meta/episodes/chunk-000/file-000.parquet).

    Returns a dict mapping episode_index → row dict, compatible with get_episode_data_index.
    Each row contains at minimum: episode_index, tasks, length, dataset_from_index,
    dataset_to_index, data/chunk_index, data/file_index, videos/{key}/chunk_index,
    videos/{key}/file_index, videos/{key}/from_timestamp, videos/{key}/to_timestamp.
    """
    import pandas as pd
    paths = sorted((local_dir / EPISODES_DIR_V30).glob("*/*.parquet"))
    if not paths:
        raise FileNotFoundError(f"No episodes parquet found in {local_dir / EPISODES_DIR_V30}")
    # Drop stats/* and tasks columns — arrays not needed for data loading
    import numpy as np
    dfs = []
    for p in paths:
        df_part = pd.read_parquet(p)
        drop_cols = [c for c in df_part.columns if c.startswith("stats/") or c == "tasks"]
        dfs.append(df_part.drop(columns=drop_cols))
    ...
    for _, row in df.iterrows():
        ...
        episodes[ep_idx]["length"] = int(row["length"])
    return episodes
