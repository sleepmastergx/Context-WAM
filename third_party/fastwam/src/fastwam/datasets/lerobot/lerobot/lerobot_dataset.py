...
from .datasets.utils import (
    DEFAULT_FEATURES,
    DEFAULT_IMAGE_PATH,
    DEFAULT_DATA_PATH_V30,
    DEFAULT_VIDEO_PATH_V30,
    INFO_PATH,
    TASKS_PATH,
...
def dataset_dir_to_repo_id(dataset_dir: str | Path) -> str:
    """Return a valid repo id while preserving local paths as dataset roots.

    LeRobot accepts a local ``root`` and a Hub-style ``repo_id``. Several local
    training configs pass paths such as ``./data/libero_...`` as
    ``dataset_dirs``; using that path string as ``repo_id`` trips HuggingFace
    validation even though the dataset is local. For local paths, use the leaf
    directory name as a stable, valid dataset id while keeping the original path
    for ``root``.
    """
    ds_path = Path(dataset_dir)
    ds_str = str(dataset_dir)
    if ds_path.is_absolute() or ds_str.startswith(".") or ds_path.exists():
        return ds_path.name
    return ds_str
...
class LeRobotDatasetMetadata:
    ...
    def get_data_file_path(self, ep_index: int) -> Path:
        if self._version >= packaging.version.parse("v3.0"):
            ...
            fpath = DEFAULT_DATA_PATH_V30.format(
                chunk_index=int(ep["data/chunk_index"]),
    ...
    def get_video_file_path(self, ep_index: int, vid_key: str) -> Path:
        if self._version >= packaging.version.parse("v3.0"):
            ep = self.episodes[ep_index]
            fpath = DEFAULT_VIDEO_PATH_V30.format(
                video_key=vid_key,
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        # v3.0 RoboTwin task shards keep global episode ids. When episodes is None,
        # range(total_episodes) is wrong for every shard after the first and also
        # disables parquet filtering. Default to the ids declared by this shard.
        if self.episodes is None and self.meta._version >= packaging.version.parse("v3.0"):
            self.episodes = sorted(self.meta.episodes.keys())
        if (self.episodes is not None
                and self.meta._version >= packaging.version.parse("v2.1")
                and self.meta._version < packaging.version.parse("v3.0")):
            episodes_stats = [self.meta.episodes_stats[ep_idx] for ep_idx in self.episodes]
            self.stats = aggregate_stats(episodes_stats)

        # v3.0: Dataset.from_parquet with isin() returns rows in parquet storage order
        # (episode_index order), regardless of the order in self.episodes. Sort episodes
        # so episode_data_index aligns with the HF dataset row ordering.
        ...
        # check_timestamps_sync skipped (requires decoded column access not compatible with set_transform)
    ...
    def get_episodes_file_paths(self) -> list[Path]:
        episodes = (
            self.episodes
            if self.episodes is not None
            else sorted(self.meta.episodes.keys())
        )
        ...
        if len(self.meta.video_keys) > 0:
            ...
            fpaths += video_files
        # v3.0 multi-episode files: many episodes share one file — deduplicate
    ...
    def _load_hf_dataset_v30(self) -> datasets.Dataset:
        """v3.0 loader: multi-episode parquet files with predicate pushdown filtering."""
        import pyarrow.dataset as pa_ds
        features = get_hf_features_from_features(self.meta.features)
        paths = sorted((self.root / "data").glob("*/*.parquet"))
        if not paths:
            raise FileNotFoundError(f"No parquet files found in {self.root / 'data'}")
        ...
        hf_dataset = datasets.Dataset.from_parquet(
            [str(p) for p in paths], filters=filters, features=features
        )
        hf_dataset.set_transform(hf_transform_to_torch)
        return hf_dataset
        ...
        for key, q_idx in query_indices.items():
            if key not in self.meta.video_keys :
                ...
                if q_idx_tuple not in processed_indices:
                    # List-index access triggers set_transform and returns a batch dict.
                    # .select() returns a Dataset whose column access bypasses the transform.
                    selected_data = self.hf_dataset[list(q_idx)]
                    index_to_selected[q_idx_tuple] = selected_data
                    processed_indices.add(q_idx_tuple)
                else:
                    selected_data = index_to_selected[q_idx_tuple]
                col = selected_data[key]
                result[key] = col if isinstance(col, torch.Tensor) else torch.stack(col)
        return result
    ...
    def _query_videos(self, query_timestamps: dict[str, list[float]], ep_idx: int) -> dict[str, torch.Tensor]:
        ...
        for vid_key, query_ts in query_timestamps.items():
            ...
            if self.meta._version >= packaging.version.parse("v3.0"):
                # In v3.0, episodes are concatenated into shared video files.
                # Timestamps in the parquet are episode-relative; shift by from_timestamp
                # to get the seek position within the concatenated file.
                from_ts = float(self.meta.episodes[ep_idx][f"videos/{vid_key}/from_timestamp"])
                query_ts = [from_ts + ts for ts in query_ts]
...
class MultiLeRobotDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        ds_names = [dataset_dir_to_repo_id(ds_dir) for ds_dir in dataset_dirs]
        ...
        try:
            ...
            logging.warning(
                "Failed to aggregate metadata stats across datasets; using the first "
                "dataset stats instead. Error: %s",
                err,
            )
            self.stats = self._datasets[0].meta.stats
    ...
    def get_episode_data(self, episode_idx: int) -> dict:
        for dataset in self._datasets:
            if episode_idx < dataset.num_episodes:
                actual_ep_idx = (
                    dataset.episodes[episode_idx]
                    if dataset.episodes is not None
                    else sorted(dataset.meta.episodes.keys())[episode_idx]
                )
                file = str(dataset.root / dataset.meta.get_data_file_path(actual_ep_idx))
                if dataset.meta._version >= packaging.version.parse("v3.0"):
                    # v3.0: shared files contain multiple episodes; filter to this one
                    table = pq.read_table(file, filters=[("episode_index", "=", actual_ep_idx)])
                else:
