...
class BaseLerobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        episode_indices_path: Optional[str] = None,
        episode_indices_key: Optional[str] = None,
        ...
        if self.image_only and not self.image_sample_indices:
            raise ValueError("image_only mode requires nonempty image_sample_indices.")
        metas = []
        for ds_dir in dataset_dirs:
            ds_root = Path(ds_dir)
            repo_id = dataset_dir_to_repo_id(ds_dir)
        ...
        self.episode_indices = self._load_episode_indices(
        ...
        image_offsets = (
            ...
            else self.image_sample_indices
        )
        ...
        if self.episode_indices is not None:
            requested = sorted(set(int(i) for i in self.episode_indices))
            for meta in metas:
                available = set(meta.episodes.keys())
                valid = [i for i in requested if i in available]
                missing = len(requested) - len(valid)
                if missing:
                    logger.warning(
                        "Ignoring %d episode indices not present in %s",
                        missing,
                        meta.repo_id,
                    )
    ...
    @staticmethod
    def _load_episode_indices(
        *,
        episode_indices: Optional[Sequence[int]],
        episode_indices_path: Optional[str],
        episode_indices_key: Optional[str],
    ) -> Optional[list[int]]:
        if episode_indices is not None and episode_indices_path is not None:
            raise ValueError("Only one of `episode_indices` and `episode_indices_path` can be set.")
        if episode_indices is not None:
            return [int(i) for i in episode_indices]
        if episode_indices_path is None:
            return None

        path = Path(episode_indices_path)
        with path.open("r", encoding="utf-8") as f:
            payload = json.load(f)
        if episode_indices_key is not None:
            if episode_indices_key not in payload:
                raise KeyError(f"`{episode_indices_key}` not found in episode split file: {path}")
            payload = payload[episode_indices_key]
        if isinstance(payload, dict) and "episodes" in payload:
            payload = payload["episodes"]
        if not isinstance(payload, list):
            raise ValueError(
                f"Episode split in {path} must be a list, or a dict containing an `episodes` list."
            )
    ...
    def __getitem__(self, idx):
        ...
        # Cache construction and cache-backed training require index identity:
        # a requested global index must never silently resolve to another
        # sample. Other existing datasets retain the historical retry behavior.
        sample_idx = idx
        attempt = 0
        last_exception: Optional[Exception] = None
        strict_getitem = bool(getattr(self, "strict_getitem", False))
        max_attempts = 1 if strict_getitem else MAX_GETITEM_ATTEMPT
        while attempt < max_attempts:
            try:
                ...
                last_exception = err
                if strict_getitem:
                    raise RuntimeError(f"Strict sample load failed for index {idx}.") from err
                logger.warning(
        ...
        image_only = bool(getattr(self, "image_only", False))
        if not image_only:
            for meta in self.state_meta:
        ...
        if bool(getattr(self, "return_images", True)):
            for meta in self.image_meta:
        ...
        if self.processor is not None:
            if image_only:
                pixel_values = self.processor.preprocess_images(
                    sample["images"],
                    expected_num_obs_steps=len(self.image_sample_indices),
                )
                sample = {
                    "idx": sample["idx"],
