#!/bin/bash
# Keep the Fast-WAM original 30x30 MoveCube run alive until it finishes:
...
# Meant to run inside tmux:  tmux new -d -s fastwam-supervisor bash scripts/supervise_original_30x30.sh
set -u
cd "$(dirname "$0")/.."

PYTHON=${PYTHON:-/workspace/venv/bin/python}
OUT_DIR=${OUT_DIR:-/workspace/outputs/fastwam_original_30x30_ddp}
HF_REPO=${HF_REPO:-SleepMastger/movecube-fastwam-original-30x30}
FINAL_STEP=${FINAL_STEP:-64260}
...
train_running()   { pgrep -f "train.py --arm original .*--out $OUT_DIR" >/dev/null; }
...
training_done() {
    grep -qs "done: $FINAL_STEP steps" "$TRAIN_LOG" "$OUT_DIR"/train_resume*_stdout.log 2>/dev/null && return 0
...
path = hf_hub_download(repo, newest, local_dir=str(out_dir))
print(path)
PY
}
...
launch_training() {
    ...
    setsid nohup bash scripts/launch_original_30x30.sh "${args[@]}" >> "$TRAIN_LOG" 2>&1 < /dev/null &
    disown
}
...
restarts=0
last_launch=0
log "supervisor started (pid $$) final_step=$FINAL_STEP out=$OUT_DIR"
while true; do
    if training_done; then
        log "training reached step $FINAL_STEP"
        # wait for the watcher to ship the final checkpoint
    ...
    if ! train_running; then
        ...
        if [ $((now - last_launch)) -lt 900 ]; then
            log "training died within 15 min of launch; backing off 10 min before retrying"
            sleep 600
        fi
        ckpt=$(latest_local_ckpt 2>>"$SUP_LOG")
        if [ -z "$ckpt" ]; then
            log "no complete local checkpoint; fetching newest from HF"
            ckpt=$(download_latest_hf_ckpt 2>>"$SUP_LOG")
        fi
        [ -n "$ckpt" ] && log "resume checkpoint: $ckpt (step $(basename "$ckpt" .pt | sed 's/step_//'))"
